...
async def async_setup_entry(
    ...
    # Retrieve wipe_data flag from hass.data safely
    wipe_data = hass.data.get(DOMAIN, {}).get(entry.entry_id, {}).get("wipe_data", False)

    """Set up the Solar Savings sensors."""
    
    # Safely coerce Configuration floats
    try:
        on_peak = float(entry.options.get(CONF_ON_PEAK_RATE, entry.data.get(CONF_ON_PEAK_RATE, 0.0)) or 0.0)
    except (ValueError, TypeError):
        on_peak = 0.0

    try:
        off_peak = float(entry.options.get(CONF_OFF_PEAK_RATE, entry.data.get(CONF_OFF_PEAK_RATE, 0.0)) or 0.0)
    except (ValueError, TypeError):
        off_peak = 0.0

    try:
        export_rate = float(entry.options.get(CONF_EXPORT_RATE, entry.data.get(CONF_EXPORT_RATE, 0.0)) or 0.0)
    except (ValueError, TypeError):
        export_rate = 0.0
    ...
    # Helper to safe get float from options without crashing on None
    def get_hist(key):
        val = entry.options.get(key, entry.data.get(key, 0.0))
        try:
    ...
    # Tracking for ROI feature
    total_savings_sensor = None
    ...
    if gen_entity and imp_entity and exp_entity:
        ...
        for period_key, period_name in PERIODS.items():
            ...
            # Keep track of Total Savings for ROI calculations
            if period_key == "total":
                total_savings_sensor = s7
        ...
        # --- ROI Setup ---
        pto_date_str = entry.options.get(CONF_PTO_DATE, entry.data.get(CONF_PTO_DATE))
        system_cost_raw = entry.options.get(CONF_SYSTEM_COST, entry.data.get(CONF_SYSTEM_COST))

        # Safely parse system cost to prevent string comparison crashes
        try:
            system_cost = float(system_cost_raw) if system_cost_raw is not None else 0.0
        except (ValueError, TypeError):
            system_cost = 0.0

        if pto_date_str and system_cost > 0 and total_savings_sensor:
            try:
                # Ensure date is valid format before setting up
                date.fromisoformat(pto_date_str)
                
                s_days = SolarSavingsROISensor(hass, entry.entry_id, "Days on Solar", "days_on_solar", pto_date_str, system_cost, total_savings_sensor, "days")
...
class SolarSavingsHistorySensor(SensorEntity):
    ...
    def __init__(self, hass, entry_id, name, unique_id, parent_sensor, mode):
        ...
        # Inherit unit and class from parent safely
        ...
        # Safely handle parent icon
        if hasattr(parent_sensor, "icon") and parent_sensor.icon:
            self._attr_icon = parent_sensor.icon.replace("calendar-today", "calendar-arrow-left")
        else:
    ...
    def update_from_history(self, history_list):
        ...
        if not history_list:
            ...
            # Safely cast history to float to prevent crashing on corrupt state data
            self._attr_native_value = sum(float(x) for x in history_list if x is not None)
...
class SolarSavingsAccumulator(RestoreSensor):
    ...
    def extra_state_attributes(self):
        ...
        attrs = {
            "last_reset": self._last_reset.isoformat() if self._last_reset else dt_util.now().isoformat(),
    ...
    async def async_added_to_hass(self) -> None:
        ...
        # 2. Calculate Final Value safely
        self._attr_native_value = (self._accumulated_delta or 0.0) + (self._initial_value or 0.0)
    ...
    def _check_reset(self):
        ...
        if self._period == "total" or not self._last_reset:
            return
        ...
        if reset:
            if self._period == "daily":
                final_val = (self._accumulated_delta or 0.0) + (self._initial_value or 0.0)
...
class SolarSavingsFinancialAccumulator(SolarSavingsAccumulator):
    ...
    def __init__(self, hass, entry_id, name, unique_suffix, source_entity_id, 
                 schedule_id, on_peak, off_peak, export_rate, mode, period, initial_value=0.0, wipe_data=False):
        super().__init__(hass, entry_id, name, unique_suffix, source_entity_id, "mdi:currency-usd", period, initial_value, wipe_data)
        self._schedule_id = schedule_id
        self._on_peak = on_peak
        self._off_peak = off_peak
        self._export_rate = export_rate
        self._mode = mode
        # Strict failback to USD if user doesn't have a standard currency setup
        self._attr_native_unit_of_measurement = hass.config.currency or "USD" 
...
class SolarSavingsSavingsAccumulator(RestoreSensor):
    ...
    def __init__(self, hass, entry_id, name, unique_suffix, 
                 gen_entity, exp_entity, schedule_id, on_peak, off_peak, export_rate, period, initial_value=0.0, wipe_data=False):
        self.hass = hass
        self._entry_id = entry_id
        self._attr_name = name
        self._attr_unique_id = f"{entry_id}_{unique_suffix}"
...
class SolarSavingsSelfConsumptionFinancialAccumulator(RestoreSensor):
    ...
    def __init__(self, hass, entry_id, name, unique_suffix, 
                 gen_entity, exp_entity, schedule_id, on_peak, off_peak, period, initial_value=0.0, wipe_data=False):
        self.hass = hass
        self._entry_id = entry_id
        self._attr_name = name
        self._attr_unique_id = f"{entry_id}_{unique_suffix}"
    ...
    def _check_reset(self):
        if self._period == "total" or not self._last_reset: return
        now = dt_util.now()
        reset = False
...
class SolarSavingsSelfConsumptionSensor(RestoreSensor):
    ...
    def __init__(self, hass, entry_id, name, unique_suffix, gen_entity_source, exp_entity_source, period, initial_value=0.0, wipe_data=False):
        self.hass = hass
        self._entry_id = entry_id
        self._attr_name = name
        self._attr_unique_id = f"{entry_id}_{unique_suffix}"
...
# --- ROI Sensors ---
class SolarSavingsROISensor(SensorEntity):
    _attr_has_entity_name = True
    ...
    def __init__(self, hass, entry_id, name, unique_suffix, pto_date_str, system_cost, parent_sensor, sensor_type):
        ...
        try:
            self._pto_date = date.fromisoformat(pto_date_str) if pto_date_str else None
        except ValueError:
            self._pto_date = None
            
        try:
            self._system_cost = float(system_cost) if system_cost is not None else 0.0
        except (ValueError, TypeError):
            self._system_cost = 0.0
    ...
    def native_value(self):
        if not self._pto_date or not self._system_cost:
            return None

        days_on_solar = (dt_util.now().date() - self._pto_date).days
        
        if days_on_solar <= 0:
            return 0

        if self._sensor_type == "days":
            return days_on_solar

        total_savings = self._parent.native_value
        if not total_savings or total_savings <= 0:
            return None

        avg_daily_savings = total_savings / days_on_solar
...
class SolarSavingsExportSensor(SensorEntity):
    ...
    def __init__(self, hass: HomeAssistant, name: str, value: float, entry_id: str, unique_suffix: str, mode: str) -> None:
        ...
        if mode == "dollars":
            ...
            self._attr_native_value = safe_value / 100.0
...
class SolarSavingsCurrentRateSensor(SensorEntity):
    ...
    def _update_state(self) -> None:
        ...
        if self._mode == "dollars":
            self._attr_native_value = (current_rate_cents or 0.0) / 100.0
        else:
